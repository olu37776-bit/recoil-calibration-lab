...
V0.7可用性见docs/USABILITY.md。界面进度不是执行许可；偏好不得保存授权、运行窗口、使能开关。自动衔接只允许计算，不能自动开始输入。

V0.8可用性操作见docs/USABILITY_V08.md。只有未补偿训练可显式成组采集，每轮需用户新操作且NoMotionDesktop禁止任何鼠标移动；响应和验证不批量自动启动。组内共用原始会话身份，不伪造独立验证。前置检查与诊断绝不产生执行许可。原有门禁、保护和用户数据范围不变。
