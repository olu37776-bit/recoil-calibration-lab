...
def self_test(output: Path) -> None:
    ...
    with TemporaryDirectory() as td, LabServer(0, data_directory=Path(td)) as server:
        ...
        try:
            for page in ('/workbench.html','/workbench.js','/teaching-ui.js','/api/catalog'):
    ...
    output.write_text(json.dumps({'status':'PASS','version':'0.6.0-rc1','platform':platform.platform(),
