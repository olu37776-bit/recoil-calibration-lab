...
let polling=false;
async function checkNative(){if(polling)return;polling=true;try{const s=await api('status');$('nativeStatus').textContent=`${s.message} ${Math.round((s.progress||0)*100)}%`;if(window.TeachingUI)TeachingUI.status(s);if(s.state!=='RUNNING'){clearInterval(poll);poll=null;say(s.message,s.state==='ERROR');if(s.preview){const next=new Image();next.onload=()=>{image=next;const c=$('previewCanvas');c.width=Math.min(900,image.width);c.height=Math.round(image.height/image.width*c.width);roi=null;drawPreview();};next.src='data:image/png;base64,'+s.preview.image;}await refreshCurrent();}}catch(e){say(e.message,true);}finally{polling=false;}}
