...
function describe(item){const u=item.unlock;return `${item.name}｜${catalog.roles[u.role]}${u.level??'?'}级 / 解锁${price(u.fee)} / 购买${price(item.price)}`;}
...
function render(data){current=data;$('summary').replaceChildren();let p=document.createElement('p');p.textContent=`当前：${data.name}\n${data.demo?'合成演示':'真实采样项目'}\n配置：${data.context.weapon} / ${data.context.pose}`;$('summary').append(p);
 ...
 const active=data.profiles.find(v=>v.id===data.active_profile);$('report').textContent=active?.report?`${active.report.passed?'门禁通过':'未通过'} / ${active.report.evidence_kind} / 游戏实测：未认证`:'尚无验证报告';$('details').textContent=JSON.stringify(active?.report||data.response||{message:'先采集响应与训练数据'},null,2);drawCurve(data.curve);$('projects').value=data.id;
}
...
async function native(mode){requireProject();const consent=mode==='execute'?$('executeConsent').checked:$('consent').checked;const status=await api('native',{mode,consent,hwnd:+$('windows').value,roi,duration_s:+$('duration').value});$('nativeStatus').textContent=status.message;say(status.message);if(poll)clearInterval(poll);poll=setInterval(checkNative,750);}
let polling=false;
async function checkNative(){if(polling)return;polling=true;try{const s=await api('status');$('nativeStatus').textContent=`${s.message} ${Math.round((s.progress||0)*100)}%`;if(s.state!=='RUNNING'){clearInterval(poll);poll=null;say(s.message,s.state==='ERROR');if(s.preview){const next=new Image();next.onload=()=>{image=next;const c=$('previewCanvas');c.width=Math.min(900,image.width);c.height=Math.round(image.height/image.width*c.width);roi=null;drawPreview();};next.src='data:image/png;base64,'+s.preview.image;}await refreshCurrent();}}catch(e){say(e.message,true);}finally{polling=false;}}
function bind(id,fn){$(id).addEventListener('click',async()=>{const b=$(id);b.disabled=true;try{await fn();}catch(e){say(e.message,true);}finally{b.disabled=false;}});}
...
bind('create',async()=>{render(await api('create',{name:$('name').value,preset:preset()}));await projects();tab('capture');say('项目已保存；请核对目标窗口与分辨率');});
...
bind('listwindows',async()=>{const r=await api('windows');$('windows').replaceChildren();for(const w of r.windows)option($('windows'),w.handle,`${w.title} · ${w.resolution.join('×')}`);say('按实际窗口尺寸创建项目；采集前需切到该窗口');});
...
async function init(){token=(await(await fetch('/api/bootstrap')).json()).token;catalog=await(await fetch('/api/catalog')).json();Object.values(catalog.weapons).forEach(w=>option($('weapon'),w.id,describe(w)));$('weapon').value='galil';slots();for(const k of catalog.slots)$(k).addEventListener('change',()=>{if(k==='sight')zoom();cost();});const env=await api('environment');$('notice').textContent=env.notice;$('environment').textContent=`${env.platform} · ${env.version} · 数据只存本机`;
 ...
 await projects();say('准备就绪。先选已有项目或创建配装。');}
