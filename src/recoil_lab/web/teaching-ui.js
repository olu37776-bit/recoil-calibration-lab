...
window.TeachingUI=(()=>{
 ...
 function showBank(next){bank=next;boxes=next?next.rois.map(x=>x.slice()):[];draw();$('teachSamples').replaceChildren();if(!next){$('teachReport').textContent='尚未建立识别库';return;}
 ...
 async function refresh(selected){banks=(await api('teach_list')).banks;const id=selected||bank?.id;$('teachBanks').replaceChildren();option($('teachBanks'),'','新建识别库');for(const b of banks)option($('teachBanks'),b.id,b.name);if(id){$('teachBanks').value=id;showBank(banks.find(b=>b.id===id)||null);}}
 ...
 bind('teachNewSession',async()=>{$('teachSession').value='session-'+crypto.randomUUID().slice(0,12);say('请实际开始一次新的采集；同一录像不能仅改会话名称');});
