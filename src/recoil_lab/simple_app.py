...
class Application:
    def __init__(self,root,directory=None):
        ...
        ttk.Label(bottom,textvariable=self.status,foreground='#134a76',wraplength=610).pack(side='left',fill='x',expand=True)
        ...
        ttk.Button(row,text='压过头了  −5',command=lambda:self.adjust(-5)).pack(side='left',padx=4)
        ttk.Button(row,text='还往上飘  ＋5',command=lambda:self.adjust(5)).pack(side='left',padx=4)
        ...
        row=ttk.Frame(third);row.pack(fill='x')
        self.start_button=ttk.Button(row,text='开始试用（5秒后）',style='Primary.TButton',command=self.start)
        self.start_button.pack(side='left',expand=True,fill='x',padx=(0,7))
        ttk.Button(row,text='立即停止  F8 / Esc',style='Primary.TButton',command=self.stop).pack(side='left',expand=True,fill='x')
        ...
        self.silent=False;self.refresh_saved();self.timer=root.after(80,self.pulse)
    ...
    def slider(self,value):
        val=round(float(value));self.rate.set(val);self.amount.set(str(val));self.edited()
    def adjust(self,delta):self.slider(max(0,min(400,self.rate.get()+delta)))
    def setting(self):
        for key,combo in self.combos.items():
            if key!='weight' and self.fields[key].get() not in combo['values']:raise ValueError('请重新选择配装项目：'+key)
        return Setting({k:v.get().strip() for k,v in self.fields.items()},round(self.rate.get()),float(self.duration.get()))
    def label(self,s):
        c=s.conditions;return f"{c['weapon']} · {c['pose']} · {c['weight']} · {c['sight']} · {s.key[:6]}"
    ...
    def save(self):
        try:
            ...
            self.saved_combo.current(next(i for i,(key,_) in enumerate(self.saved_values) if key==s.key));self.status.set('已保存手动参数；重开程序后从上方选择即可')
        except (ValueError,OSError) as e:self.error(e)
    def apply(self,s):
        self.runner.stop('已载入手动参数，尚未启用');self.silent=True
        self.fields['weapon'].set(s.conditions['weapon']);self.weapon_changed()
        for k,v in s.conditions.items():self.fields[k].set(v)
        self.rate.set(s.rate);self.amount.set(str(round(s.rate)));self.duration.set(f'{s.duration:g}')
        self.silent=False;self.consent.set(False);self.target=None;self.window.set('');self.status.set('已载入手动参数；重新核对窗口并确认后才可试用')
    def load_selected(self,event=None):
        i=self.saved_combo.current()
        if i>=0:
            try:self.apply(self.saved_values[i][1])
    ...
    def undo(self):
        try:
            ...
            self.apply(selected);self.status.set('已恢复保存值；需要时重新试用，再保存')
        except ValueError as e:self.error(e)
    ...
    def close(self):self.runner.stop('已退出');self.root.after_cancel(self.timer);self.root.destroy()
...
def self_test(out):
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        try:
            ...
            app.close();root=tk.Tk();app=Application(root,directory);root.update()
            assert len(app.store.items())==2 and not app.runner.active and not app.consent.get()
            result.update(status='PASS',checks=['native_tk_window','manual_adjust','save','previous_value','weapon_compatibility','manual_weight','2560x1440_feedback','restart_without_permission'])
        finally:
            try:app.close()
